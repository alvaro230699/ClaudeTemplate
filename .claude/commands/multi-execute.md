...
5. **Apply Changes**:
   - **Path traversal guard (mandatory):** Before applying any hunk, verify that every target path in the diff resolves inside `$PWD`. Reject the entire diff if any path contains `..` sequences or resolves outside the working directory.
   - **User confirmation (mandatory):** Present the full diff to the user and require explicit confirmation before writing any file. Do not apply changes automatically.
   - Use Edit/Write tools to execute actual modifications only after confirmation.
